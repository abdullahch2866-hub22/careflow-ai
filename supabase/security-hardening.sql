-- CareFlow AI security hardening snapshot
-- Applied to the production Supabase project on 2026-09-01.
-- This file documents the database-side protections that accompany the deployed frontend and Edge Functions.

-- 1) Source PDFs are not directly readable through the Storage API.
-- Upload remains allowed by the existing organization-scoped INSERT policy.
drop policy if exists "Hospital members can view their own documents flreew_0" on storage.objects;

-- 2) Full case audit snapshots stay private. The public compatibility view exposes only
-- changed-field markers so the Activity Log does not download patient values to the browser.
do $$
begin
  if to_regclass('public.case_review_history_private_data') is null
     and to_regclass('public.case_review_history') is not null then
    alter table public.case_review_history rename to case_review_history_private_data;
  end if;
end $$;

revoke all on table public.case_review_history_private_data from anon, authenticated;
...
begin
  ...
  insert into public.case_review_history_private_data
...
create or replace view public.case_review_history
...
from public.case_review_history_private_data h
where exists (
  select 1
...
-- 3) Least-privilege direct browser grants.
...
-- Retire the older organization-members RPC. The frontend uses v2.
revoke execute on function public.careflow_my_organization_members() from authenticated;

-- 4) Indexes for tenant-scoped and FK lookups.
create index if not exists case_review_history_case_id_idx on public.case_review_history_private_data(case_id);
...
-- 5) Immutable case metadata and strict document-row creation.
